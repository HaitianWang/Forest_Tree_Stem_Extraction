...
function pc_classify(rootTLSData,rootResults)
% --------------------------------------------------------------------------------------------    
    % starttime = datetime;
...
%     disp(list)
%     list = ("/home/ibrahim/Downloads/Point-Cloud-Tools-main/PCT/tmp/FPCBZ25_03_Output_laz1_4_clipped.las");
    
%     filenames = "/home/ibrahim/Downloads/Point-Cloud-Tools-main/PCT/FPCBZ42_01_Output_laz1_4_clipped.las"
    warning('off','MATLAB:mir_warning_maybe_uninitialized_temporary');
    names = dir("/media/ibrahim/9cfd0642-7019-4875-9519-d9bd91f84711/FORESTDATARESULTS23/FPCBZ42_01_Output_laz1_4/SegmentedTrees");
    list = [];
    for j=3:20
        file = "/media/ibrahim/9cfd0642-7019-4875-9519-d9bd91f84711/FORESTDATARESULTS23/FPCBZ42_01_Output_laz1_4/SegmentedTrees/" +names(j).name;
        list = [list; file];
    end
    parfor_progress(size(list,1));
    disp((list))

    tic
    parfor i = 1:size(list,1)
        try
            PCtls = lasdata(list(i));
...
%                 pc = pcdownsample(pointCloud(PCtls.Location(PCtls.Location(:,3)<=3,:)),'gridAverage',0.005);
                pc = (pointCloud(PCtls.Location(PCtls.Location(:,3)<=2,:)));
                pc = pointCloud(pc.Location(pc.Location(:,3) >= 0,:));
                    ...
                    for j = 1:size(cylmodels,1)
                        ...
                        if ~isempty(pc.Location)
                            ...
                            if ~isempty(pc.Location)
                                ...
                                % Save stem points
                                savedir = [rootResults,'/pointclouds/stem'];
                                if isfolder(savedir) == false; mkdir(savedir);end
                                writematrix(cat(2,inPoints.Location,numTrees.*ones([inPoints.Count,1])),[savedir,sprintf('/tree%i_stempoints.txt',numTrees)],'Delimiter','\t');
                                ...
                                % Save stem points and crown points
                                if ~isempty(canopypoints.Location)
                                    savedir = [rootResults,'/pointclouds/canopy'];
                                    if isfolder(savedir) == false; mkdir(savedir); end
                                    writematrix(cat(2,canopypoints.Location,numTrees.*ones([canopypoints.Count,1])),[savedir,sprintf('/tree%i_canopypoints.txt',numTrees)],'Delimiter','\t');
                                end
                            end
                        end
                    end
